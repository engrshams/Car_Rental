...
const filteredData = computed(() => {
    ...
    let filtered = props.data.filter((row) => {
        return props.columns
            ...
            .some((column) => {
                // Handle nested fields and slots
                let value;
                // const value = String(row[column.key]).toLowerCase();
                // return value.includes(query);
                if (column.key === 'customer') {
                    value = row.customer?.cus_name ?? '';
                } else if (column.key === 'car') {
                    value = row.car?.name ?? '';
                } else {
...
<template>
    <div class="bg-white shadow-md rounded-lg overflow-hidden">
        <!-- Search Input -->
        <div class="p-2">
            <input v-model="searchQuery" type="text" placeholder="Search by name, car or cost..."
                class="w-full p-1 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500" />
        </div>
